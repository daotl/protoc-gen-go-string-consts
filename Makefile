...
PROJECTNAME="protoc-gen-go-string-consts"
...
.PHONY: install
## `install`: Fetch dependencies needed to run `protoc-gen-go-string-consts`
install:
...
.PHONY: run
## :
## `run`: Run `protoc-gen-go-string-consts` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `protoc-gen-go-string-consts` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `protoc-gen-go-string-consts`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `protoc-gen-go-string-consts`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
