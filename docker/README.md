...
a docker containers for *protoc-gen-go-string-consts*!
...
```bash
make docker-gen IMAGE=protoc-gen-go-string-consts VERSION=latest
```

By default, the values for these environment variables are; <br>

```bash
IMAGE := protoc-gen-go-string-consts
VERSION := latest
```
...
```bash
docker run protoc-gen-go-string-consts
```
...
```bash
make clean-docker IMAGE=protoc-gen-go-string-consts VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *protoc-gen-go-string-consts* uses
...
```bash
make docker-debug IMAGE=protoc-gen-go-string-consts-debug VERSION=latest
```
...
```bash
docker run protoc-gen-go-string-consts-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it protoc-gen-go-string-consts-debug bash
```
...
```bash
make clean-docker IMAGE=protoc-gen-go-string-consts-debug VERSION=latest
```
