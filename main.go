...
import (
	...
	"github.com/daotl/protoc-gen-go-string-consts/options"
)
...
func generateFile(gen *protogen.Plugin, file *protogen.File) {
	...
	for _, enum := range file.Enums {
		...
		if !proto.HasExtension(opts, options.E_GenerateGoStringConsts) {
			continue
		}

		if enabled, ok := proto.GetExtension(opts, options.E_GenerateGoStringConsts).(bool); !ok {
	...
	for _, enum := range file.Enums {
		...
		if proto.HasExtension(opts, options.E_GenerateGoStringConstsStripNamePrefix) {
			var ok bool
			if stripNamePrefix, ok = proto.GetExtension(opts,
				options.E_GenerateGoStringConstsStripNamePrefix).(bool); !ok {
				log.Fatalf(
		...
		if proto.HasExtension(opts, options.E_GenerateGoStringConstsStripValuePrefix) {
			var ok bool
			if stripValuePrefix, ok = proto.GetExtension(opts,
				options.E_GenerateGoStringConstsStripValuePrefix).(bool); !ok {
				log.Fatalf(
